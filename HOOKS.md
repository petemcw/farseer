...
This is the authoritative reference for Farseer's hook system. Farseer uses
...
(`planner`, `orchestrate`) point here rather than duplicating this
logic.
...
| Hook                  | Fires                                                                  |
|-----------------------|------------------------------------------------------------------------|
| `pre-plan`            | Before `planner` Phase 1 (Discovery) begins                            |
| `post-plan`           | `planner` Phase 6, after dependency validation                         |
| `pre-implementation`  | `orchestrate` after Step 2, before the first batch                     |
| `pre-batch`           | Each loop iteration, before Step 5 (spawn workers)                     |
| `post-batch`          | Each loop iteration, after Step 6 (collect results)                    |
| `post-implementation` | `orchestrate` Step 4a, when all tasks are complete                     |
...
There is deliberately **no** `execution` hook. In Farseer, executing a plan **is**
the implementation loop — the Step 3 → 5 → 6 → 8 batch cycle in
`orchestrate`. An `execution` hook would fire over that same span already
...
| `mode`  | enum    | `single`   | How the agent is spawned: `single` (one sub-agent for the whole plan) or `batch` (files are split into batches of ~10 and spawned as parallel sub-agents). |

### The enrollment rule

**If an agent declares a `phase`, it runs.** There is no conditional
evaluation. Farseer intentionally has **no** `requires:` or `condition:` mechanism —
...
script exists to eliminate: the routine used to be prose, every session improvised
its own bash, one of those scripts had a syntax error, and the partial output was
consumed as a complete enumeration. Every hook silently resolved empty.
...
Spawn modes: `single` → one sub-agent for the whole plan. `batch` → split the
relevant file list into batches of ~10 and spawn parallel sub-agents (one Task
...
2. **Resolve the project root,** in order: `CLAUDE_PROJECT_DIR` when set (and a
   ...
   nearest ancestor of the working directory holding a `.farseer/`; else the
   enclosing Git repository root, since a project that has not created
   `.farseer/` yet is still a project. `$HOME` bounds the last two, so neither
...
3. **Enumerate local agents.** Top-level `*.md` in `<project root>/.farseer/agents`.
...
`planner` writes this to `.farseer/plans/{plan-name}/.hook-fingerprint`, and
`orchestrate` passes it back as `--expect=<value>` on every hook call. If
enrollment changed, the script exits **4** and names what changed; Farseer halts.
...
re-baseline, which the exit-4 message says. Nothing else Farseer does mid-run
touches `agents/` or `.farseer/agents/`.
...
- Do **no work** — no staging, no diffing, no sub-agent spawns.
